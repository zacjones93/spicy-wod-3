...
export function getAppPort({ isWorker = true } = {}) {
  return isWorker ? BASE_WRANGLER_PORT : BASE_NEXT_PORT;
}
...
function configurePlaywright({
  ...
  const port = getAppPort({ isWorker });
  const inspectorPort = 9300;
...
export default configurePlaywright({
  isWorker: true, // Test against Cloudflare Worker
  multipleBrowsers: false, // Start with just Chrome
