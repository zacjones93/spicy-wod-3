...
export async function submitLogFormAction(
  ...
  if (parsedScoreEntries.length === 0 || !atLeastOneScorePartFilled) {
    if (workout.scheme !== "N/A" && workout.scheme !== undefined) { // N/A scheme might not require scores
      console.error("[Action] No valid score parts provided for a workout that expects scores.");
  ...
  for (let k = 0; k < parsedScoreEntries.length; k++) {
    ...
    if (isRoundsAndRepsWorkout) {
      ...
      if (scoreParts.length < 2 && scoreParts[0].trim() === "") { // If first part (rounds) is empty, skip this set potentially or error
      ...
      const roundsCompleted = parseInt(roundsStr, 10);
      const repsCompleted = parseInt(repsStr, 10);

      if (isNaN(roundsCompleted) || (scoreParts[1] !== undefined && isNaN(repsCompleted))) {
        return { error: `Invalid number for rounds or reps for set ${setNumber}. Rounds: '${roundsStr}', Reps: '${repsStr}'.` };
      }
      if (roundsCompleted < 0 || repsCompleted < 0) {
        return { error: `Rounds and reps for set ${setNumber} cannot be negative.` };
      }
      if (workout.repsPerRound && repsCompleted >= workout.repsPerRound) {
        return { error: `Reps for set ${setNumber} (${repsCompleted}) cannot exceed or equal reps per round (${workout.repsPerRound}). Adjust rounds or reps.` };
      }

      setsForDb.push({ setNumber, roundsCompleted, repsCompleted, score: null, time: null });
      ...
      if (timeInSeconds === null) {
        return { error: `Invalid time format for set ${setNumber}: '${timeStr}'. Please use MM:SS or total seconds.` };
      }
      ...
      if (scoreStr === undefined || scoreStr.trim() === "") {
        ...
        return { error: `Score input for set ${setNumber} is missing for scheme '${workout.scheme}'.` };
      }
      const numericScore = parseInt(scoreStr, 10);
      if (isNaN(numericScore)) {
        return { error: `Score for set ${setNumber} ('${scoreStr}') must be a valid number for scheme '${workout.scheme}'.` };
      }
      if (numericScore < 0) {
        return { error: `Score for set ${setNumber} ('${numericScore}') cannot be negative.` };
      }
  ...
  if (setsForDb.length === 0 && workout.scheme !== "N/A" && workout.scheme !== undefined && atLeastOneScorePartFilled) {
    ...
    console.error("[Action] All provided score entries resulted in no valid sets to save, but some input was detected.");
    return { error: "Valid score information is required. Please check your inputs for each round/set." };
  } else if (setsForDb.length === 0 && workout.scheme !== "N/A" && workout.scheme !== undefined && !atLeastOneScorePartFilled) {
    // This repeats the earlier check, but is a failsafe after processing.
    console.error("[Action] No score entries provided or all were empty, and workout expects scores.");
    return { error: "At least one score input is required and must not be empty." };
  }
