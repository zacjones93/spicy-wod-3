...
import type { Movement, Tag, WorkoutCreate, WorkoutResult, WorkoutUpdate } from "@/types";
import { and, desc, eq, gte, inArray, lte, or } from "drizzle-orm";
import { getDbAsync } from "../db";
import { movements, results, tags, workoutMovements, workoutTags, workouts } from "../db/schema";
...
async function fetchMovementsByWorkoutId(
	...
	const movementObjects: Movement[] = movementIds.length
		? await db.select().from(movements).where(inArray(movements.id, movementIds))
		: [];
...
async function fetchTodaysResultsByWorkoutId(
	...
	if (!userId || workoutIds.length === 0) return new Map<string, WorkoutResult[]>();
...
export async function getAllWorkouts() {
	...
	const [tagsByWorkoutId, movementsByWorkoutId, resultsByWorkoutId] = await Promise.all([
...
export async function getWorkoutsByMovementId(movementId: string) {
	...
	const movementObjs = movementIdsForWorkouts.length
		? await db.select().from(movements).where(inArray(movements.id, movementIdsForWorkouts))
		: [];
